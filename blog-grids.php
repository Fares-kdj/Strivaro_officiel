...
<html lang="en" class="dark">
  <head>
    ...
    <title>Strivaro | Blog Grids</title>
...
<!-- ===== Blog Section Start ===== -->
<!-- ===== Blog Section Start ===== -->
<section id="blog" class="pt-[240px]">
      ...
      <!-- <div class="-mx-4 flex flex-wrap">
        ...
        try {
          ...
          if (!file_exists($db_include)) {
            throw new Exception("Database connection file not found: $db_include");
          }
          include $db_include;

          // Vérifier si $conn est défini
          if (!isset($conn) || !($conn instanceof PDO)) {
            throw new Exception("PDO connection not initialized in db.php");
          }

          // Query to fetch blog posts
          $stmt = $conn->query("SELECT id, image_urls, category, writer, created_at, title FROM blogs ORDER BY created_at DESC");
          $blogs = $stmt->fetchAll(PDO::FETCH_ASSOC);

          // Loop through blog posts
          $delay = 0.2; // Starting delay in seconds
          foreach ($blogs as $blog) {
            ...
            $url = "blog-details.php?id=$id"; // Generated URL
            $delay_str = number_format($delay, 1) . 's'; // Format delay as string

            // Handle image_urls (JSON-encoded array)
        ...
        ?>
          <div class="w-full px-4 md:w-1/2 lg:w-1/3">
            <div class="wow fadeInUp mb-10" data-wow-delay="<?php echo $delay_str; ?>">
              <div class="relative mb-8 overflow-hidden rounded-sm">
                    ...
                    if (!empty($image_urls)) {
                      ...
                      if (file_exists($primaryMediaPath)) {
                        ...
                        if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                          $mediaHtml = '<img src="' . htmlspecialchars($primaryMediaPath) . '" alt="' . htmlspecialchars($title) . '" class="w-full h-48 object-cover rounded-md" />';
                        } elseif (in_array($fileExtension, ['mp4', 'webm', 'ogg'])) {
                          $mediaHtml = '<video class="w-full h-48 object-cover rounded-md" controls>
                ...
                <span class="bg-primary font-heading absolute top-5 left-5 rounded-sm px-4 py-[6px] text-sm font-medium text-white">
                  <?php echo $category; ?>
                </span>
              </div>
              <div>
                ...
                <h3>
                  <a href="<?php echo $url; ?>" class="font-heading text-dark hover:text-primary dark:hover:text-primary text-xl font-medium md:text-2xl lg:text-xl xl:text-2xl dark:text-white">
                    <?php echo $title; ?>
                  </a>
                </h3>
              </div>
        ...
        <?php
          ...
          echo '<p class="text-red-500 text-center">Error fetching blog posts: ' . htmlspecialchars($e->getMessage()) . '</p>';
        }
        ?>
      </div> -->
        <div class="-mx-4 flex flex-wrap">
  ...
  try {
    ...
    if (empty($blogs)) {
      ...
      echo '<p class="font-heading text-dark-text text-lg dark:text-white">Aucun article disponible</p>';
      echo '</div>';
    } else {
      // Loop through blog posts
      $delay = 0.2; // Starting delay in seconds
        ...
        <div>
          <div class="mb-4 flex items-center">
            <p class="font-heading text-dark-text flex items-center pr-5 text-base">
              <span class="pr-2">
                <svg width="18" height="18" viewBox="0 0 18 18" class="fill-current">
                  <path d="M15 16.5H13.5V15C13.5 14.4033 13.2629 13.831 12.841 13.409C12.419 12.9871 11.8467 12.75 11.25 12.75H6.75C6.15326 12.75 5.58097 12.9871 5.15901 13.409C4.73705 13.831 4.5 14.4033 4.5 15V16.5H3V15C3 14.0054 3.39509 13.0516 4.09835 12.3484C4.80161 11.6451 5.75544 11.25 6.75 11.25H11.25C12.2446 11.25 13.1984 11.6451 13.9016 12.3484C14.6049 13.0516 15 14.0054 15 15V16.5ZM9 9.75C8.40905 9.75 7.82389 9.63361 7.27792 9.40746C6.73196 9.18131 6.23588 8.84984 5.81802 8.43198C5.40015 8.01412 5.06869 7.51804 4.84254 6.97208C4.6164 6.42611 4.5 5.84095 4.5 5.25C4.5 4.65905 4.6164 4.07389 4.84254 3.52793C5.06869 2.98196 5.40015 2.48588 5.81802 2.06802C6.23588 1.65016 6.73196 1.31869 7.27792 1.09254C7.82389 0.866396 8.40905 0.75 9 0.75C10.1935 0.75 11.3381 1.22411 12.182 2.06802C13.0259 2.91193 13.5 4.05653 13.5 5.25C13.5 6.44348 13.0259 7.58807 12.182 8.43198C11.3381 9.2759 10.1935 9.75 9 9.75ZM9 8.25C9.79565 8.25 10.5587 7.93393 11.1213 7.37132C11.6839 6.80871 12 6.04565 12 5. 你好5C12 4.45435 11.6839 3.69129 11.1213 3.12868C10.5587 2.56607 9.79565 2.25 9 2.25C8.20435 2.25 7.44129 2.56607 6.87868 3.12868C6.31607 3.69129 6 4.45435 6 5.25C6 6.04565 6.31607 6.80871 6.87868 7.37132C7.44129 7.93393 8.20435 8.25 9 8.25Z" />
                </svg>
              </span>
              <?php echo $author; ?>
            </p>
  ...
  <?php
    ...
    echo '<div class="w-full px-4 text-center">';
    echo '<p class="text-red-500 text-lg">Erreur lors de la récupération des articles : ' . htmlspecialchars($e->getMessage()) . '</p>';
    echo '</div>';
  }
  ?>
</div>
      <!-- <div class="wow fadeInUp flex items-center justify-center pt-10" data-wow-delay=".2s">
        <ul class="-mx-1 flex items-center sm:-mx-[6px]">
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">
              <span>
          ...
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">1</a>
          </li>
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">2</a>
          </li>
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">3</a>
          </li>
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">4</a>
          </li>
          <li class="px-1 sm:px-[6px]">
            <a href="javascript:void(0)" class="hover:bg-primary flex h-9 items-center justify-center rounded-sm bg-[#2C3443] px-3 py-2 text-base font-medium text-white sm:h-10 sm:px-4">
              <span>
