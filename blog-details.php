  ...
  <head>
    ...
    <?php
      try {
        ...
        $db_include = './portfolio/includes/db.php';
        if (!file_exists($db_include)) {
          throw new Exception("Fichier de connexion à la base de données introuvable : $db_include");
        }
        include $db_include;

        // Vérifier si $conn est défini
        if (!isset($conn) || !($conn instanceof PDO)) {
          throw new Exception("Connexion PDO non initialisée dans db.php");
        }

        // Récupérer l'ID du blog depuis l'URL
        $id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
        if ($id <= 0) {
          throw new Exception("ID de blog invalide.");
        }

        // Requête pour récupérer les détails du blog
        $stmt = $conn->prepare("SELECT id, title, content, image_urls, category, writer, created_at FROM blogs WHERE id = :id");
        $stmt->execute(['id' => $id]);
        $blog = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$blog) {
          throw new Exception("Blog introuvable.");
        }
        ...
        $author = htmlspecialchars($blog['writer']);
        $date = htmlspecialchars($blog['created_at']);
        $image_urls = json_decode($blog['image_urls'], true) ?: [];
        $primaryFilePath = !empty($image_urls) ? './portfolio/uploads/' . basename($image_urls[0]) : '';
    ?>
    <title><?php echo $title; ?> | Strivaro</title>
    <meta name="description" content="<?php echo substr($content, 0, 150); ?>..." />
...
<!-- ===== Header End ===== -->
    ...
    <section class="pt-[130px]">
      <div
        class="wow fadeInUp relative z-10 bg-cover bg-center bg-no-repeat py-20 lg:py-[120px]"
        data-wow-delay=".2s"
        style="background-image: url('<?php echo !empty($primaryFilePath) && file_exists($primaryFilePath) ? htmlspecialchars($primaryFilePath) : './images/blog/blog-details.jpg'; ?>');"
      >
        <div
          class="absolute top-0 left-0 z-10 h-full w-full bg-cover bg-center opacity-20 mix-blend-overlay"
          style="background-image: url('./images/blog/NoisePatern.svg');"
        ></div>
        <div
          class="absolute top-0 left-0 -z-10 h-full w-full bg-[#EEF1FDEB] dark:bg-[#1D232DD9]"
        ></div>
        <div class="px-4 xl:container">
          <div class="mx-auto max-w-[580px] text-center">
            <h1
              class="font-heading text-dark mb-8 text-3xl font-semibold md:text-[44px] md:leading-tight dark:text-white"
            >
              <?php echo $title; ?>
            </h1>
            <div class="-mx-5 flex flex-wrap items-center justify-center">
              <div class="flex items-center px-5">
                <div class="mr-4 h-10 w-10 rounded-full flex items-center justify-center bg-gray-2 dark:bg-dark-bg">
                  <svg width="24" height="24" viewBox="0 0 24 24" class="fill-current text-dark-text dark:text-white">
                    <path d="M12 12C14.21 12 16 10.21 16 8C16 5.79 14.21 4 12 4C9.79 4 8 5.79 8 8C8 10.21 9.79 12 12 12ZM12 14C9.33 14 4 15.34 4 18V20H20V18C20 15.34 14.67 14 12 14Z" />
                  </svg>
                </div>
                <p class="font-heading text-dark-text text-base dark:text-white">
                  <?php echo $author; ?>
                </p>
              </div>
              <div class="flex items-center px-5">
                <span class="text-dark-text pr-3 dark:text-white">
                  <svg
                    width="22"
                    height="22"
                    viewBox="0 0 22 22"
                ...
                <p
                  class="font-heading text-dark-text text-base dark:text-white"
                >
                  <?php echo $date; ?>
                </p>
              </div>
      ...
      <div class="px-4 pt-24 xl:container">
        <div class="border-b pb-20 lg:pb-[130px] dark:border-[#2E333D]">
          <div class="wow fadeInUp mx-auto max-w-[920px]" data-wow-delay=".2s">
            <p class="text-dark-text mb-9 text-base">
              Strivaro Blogs
            </p>
          </div>
          <?php
            $mediaHtml = '';
            if (!empty($image_urls)) {
              $firstMedia = basename($image_urls[0]);
              $primaryMediaPath = './portfolio/uploads/' . $firstMedia;

              if (file_exists($primaryMediaPath)) {
                $fileExtension = strtolower(pathinfo($firstMedia, PATHINFO_EXTENSION));
                if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                  $mediaHtml = '<img src="' . htmlspecialchars($primaryMediaPath) . '" alt="' . htmlspecialchars($title) . '" class="w-full rounded-sm" loading="lazy" />';
                } elseif (in_array($fileExtension, ['mp4', 'webm', 'ogg'])) {
                  $mediaHtml = '<video class="w-full rounded-sm" controls>
                                  <source src="' . htmlspecialchars($primaryMediaPath) . '" type="video/' . $fileExtension . '">
          ...
          ?>
          <div class="wow fadeInUp mx-auto mb-12 max-w-[1150px]" data-wow-delay=".2s">
            <?php echo $mediaHtml ?: '<img src="src/images/blog/blog-details-2.jpg" alt="blog-details" class="w-full rounded-sm" />'; ?>
          </div>
          <div class="wow fadeInUp mx-auto max-w-[920px]" data-wow-delay=".2s">
            <div class="relative mx-auto max-w-[850px] pt-12 pb-20">
              ...
              <p class="font-heading text-dark-text mb-5 text-center text-lg">
                <?php echo htmlspecialchars($content); ?>
              </p>
              <p class="font-heading text-dark-text text-center text-base">
                “<?php echo $author; ?>”
              </p>
            </div>
          </div>
          ...
          <div class="wow fadeInUp mx-auto max-w-[920px]" data-wow-delay=".2s">
            <h3
              class="font-heading text-dark mb-14 text-3xl font-medium md:text-[40px] dark:text-white"
            >
            ...
            <form>
              <div class="-mx-4 flex flex-wrap">
                <div class="w-full px-4 sm:w-1/3">
                  <div class="mb-12">
                    <input
                      ...
                      id="name"
                      placeholder="Nom"
                      class="text-dark placeholder-dark-text focus:border-primary w-full border-b bg-transparent py-5 text-base outline-hidden dark:border-[#2C3443] dark:text-white dark:focus:border-white"
                    />
                  </div>
                </div>
                <div class="w-full px-4 sm:w-1/3">
                ...
                <div class="w-full px-4">
                  <div class="mb-12">
                    ...
                    >
                      ...
                      J'ai lu les conditions de service et j'accepte la politique de confidentialité
                    </label>
                  </div>
                </div>
                ...
                <div class="w-full px-4">
                  <button
                    class="bg-primary font-heading hover:bg-primary/90 flex items-center justify-center rounded-sm px-8 py-[14px] text-base text-white"
                  >
      ...
      <?php
          ...
          echo '<section class="pt-[130px]"><div class="px-4 xl:container"><p class="text-red-500 text-center">Erreur : ' . htmlspecialchars($e->getMessage()) . '</p></div></section>';
        }
      ?>
    </section>
...
<style>
  ...
  @keyframes walkThird {
    ...
    <script defer src="https://static.cloudflareinsights.com/beacon.min.js/vcd15cbe7772f49c399c6a5babf22c1241717689176015" integrity="sha512-ZpsOmlRQV6y907TI0dKBHq9Md29nnaEIPlkf84rnaERnq6zvWvPUqr2ft8M1aS28oN72PdrCzSjY4U6VaAw1EQ==" data-cf-beacon='{"rayId":"9444bba7ab7ae19a","version":"2025.4.0-1-g37f21b1","r":1,"token":"9a6015d415bb4773a0bff22543062d3b","serverTiming":{"name":{"cfExtPri":true,"cfL4":true,"cfSpeedBrain":true,"cfCacheStatus":true}}}' crossorigin="anonymous"></script>
  </body>
</html>